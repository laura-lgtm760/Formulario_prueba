...
with st.form(key=f"formulario_parcela_{st.session_state.contador_form}", enter_to_submit=False):
    ...
    # Ponemos value=0 o 0.0 por defecto para que si no tocan nada, devuelva un número válido
    cod_mun = st.number_input("Código municipio:", step=1, value=0)
    poligono = st.number_input("Polígono:", step=1, value=0)
    parcela = st.number_input("Parcela:", step=1, value=0)
    recinto = st.number_input("Recinto:", step=1, value=0)
    super_ha = st.number_input("Superficie (Ha):", min_value=0.0, step=0.01, value=0.0)
...
if enviado:
    if not nombre.strip() or not municipio.strip():
        st.error("El nombre completo y el municipio son obligatorios.")
    else:
        st.session_state.nombre_guardado = nombre
        st.session_state.telefono_guardado = telefono
        
        # Capturamos la hora de Madrid de forma 100% segura contra invierno/verano
        ahora = datetime.now(ZoneInfo("Europe/Madrid"))
        fecha_hora_texto = ahora.strftime("%d/%m/%Y %H:%M:%S")
        
        # Salvavidas automático: Si los números vienen vacíos o dan error, los forzamos a 0
        try: v_cod_mun = int(cod_mun)
        except: v_cod_mun = 0
            
        try: v_poligono = int(poligono)
        except: v_poligono = 0
            
        try: v_parcela = int(parcela)
        except: v_parcela = 0
            
        try: v_recinto = int(recinto)
        except: v_recinto = 0
            
        try: v_super_ha = float(super_ha)
        except: v_super_ha = 0.0
        
        # Creamos la fila asegurando que los textos vacíos vayan limpios
        nueva_fila = {
            "Fecha y Hora Registro": fecha_hora_texto,
            "Nombre": nombre, 
            "Teléfono": telefono if telefono else "", 
            "Municipio": municipio,
            "Código Municipio": v_cod_mun, 
            "Polígono": v_poligono, 
            "Parcela": v_parcela,
            "Recinto": v_recinto, 
            "Superficie Ha": v_super_ha,
            "Cultivo Anterior": cultivo_anterior if cultivo_anterior else "", 
            "Cultivo Posterior": cultivo_posterior if cultivo_posterior else "",
...
if st.session_state.mostrar_boton_otra:
    ...
    with col2:
        if st.button("Terminar y Enviar"):
            ...
            # Ordenamos las columnas para poner la fecha al principio
            columnas = ["Fecha y Hora Registro"] + [col for col in df_final.columns if col != "Fecha y Hora Registro"]
            df_final = df_final[columnas]
